...
var GurantyFancing=function(){
	...
	var _init=function(){
		...
		$('#shipImport').upload({
			...
			beforeSend:GurantyFancing.validateExcel,
...
//				$("#continueInfo").css({display: "none"});
	...
	var _initShipGrid=function(){
		...
		ship_mmg.on("loadSuccess",function(e, data){
			GurantyFancing.countShipNumPrices();
		})
	};
	...
	return{
		...
		upLoadChange :function(val){
			...
			if(!GurantyFancing.validateFile(val)){
				return;
			}
