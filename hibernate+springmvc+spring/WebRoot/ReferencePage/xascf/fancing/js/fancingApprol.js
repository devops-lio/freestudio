 $(document).ready(function(){
		PUI.plugin.init({},$("#approd_form"));
		sns.valid.init($("#approd_form"));
		$("#zhangqi").find(".chosen-container").css("float",'right').css("width",'30%');
		
			$("#overdueType").val($("#fancingOverdueType").val()).trigger("liszt:updated");
			$("#rebackType").val($("#fancingRebackType").val()).trigger("liszt:updated");
			$("#rateCycle").val($("#fancingRateCycle").val()).trigger("liszt:updated");
			if($("#fancingRebackType").val()=='4'){
	...
	//初始化mmgridTab
	 $('#diya_Tab').on('shown', function (e) {
		 FancingApprolval._diyaInit();	
	 });
	 $('#danbao_Tab').on('shown', function (e) {
		 FancingApprolval._danbaoInit();	
	 });
	 $('#ladingBill_Tab').on('shown', function (e) {
		 FancingApprolval._ladingBillInit();	
	 });
	 $('#receipt_Tab').on('shown', function (e) {
		 FancingApprolval._receiptInit();	
	 });
});
 ...
 var details_ship_mmg=$('#details_ship_mmg').mmGrid({
	...
	details_ship_mmg.on("loadSuccess",function(e, data){
		FancingApprolval.countNumPrices();
	});
	...
	return {
		...
		_ladingBillInit :function(){
			...
			details_ladingBill_mmg.on("loadSuccess",function(e, data){
				FancingApprolval.countLadingBillNumPrices();
			})
		},
		...
		details_receipt_mmg.on("loadSuccess",function(e, data){
				FancingApprolval.countReceiptNumPrices();
			})
		},
		/**
		 * 计算提单总价格、总数量
		...
		saveFancingApprol :function(){
			if (!$("#approd_form").isValid()) {
				return ;
			}
			...
			var params=$("#approd_form").serializeArray();
			var i=0;
			...
			$.post("xascf/fancing/saveApplor.shtml",params,function(data){  
				if(data.indexOf('成功')>=0){
					PUI.MessageBox.info(data, null, {autoClose: true, timeOut: 997, afterClose: function() {
						Menu.forward("xascf/fancing/jsp/fancingApprovalList.jsp"); 
					}});
				}else{
