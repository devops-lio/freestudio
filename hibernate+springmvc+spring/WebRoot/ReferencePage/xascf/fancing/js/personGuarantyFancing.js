...
var PersonGurantyFancing=function(){
	...
	var _init=function(){
		...
		$('#shipImport').upload({
			...
			beforeSend:PersonGurantyFancing.validateExcel,
...
//				$("#continueInfo").css({display: "none"});
	...
	var _initShipGrid=function(){
		...
		ship_mmg.on("loadSuccess",function(e, data){
			PersonGurantyFancing.countShipNumPrices();
		})
	};
	...
	var _initReceiptGrid=function(){
		...
		receipt_mmg.on("loadSuccess",function(e, data){
			PersonGurantyFancing.countNumPrices();
		})
	};
	...
	return{
		...
		upLoadChange :function(val){
			...
			if(!PersonGurantyFancing.validateFile(val)){
				return;
			}
		...
		comfirFancingOrder : function(){
			...
			//判断发票和运单选择
			if (null == receipt_mmg.row(0)&&null== ship_mmg.row(0)) {
	...
	PersonGurantyFancing.init();
	 ...
	 if(orderNo!=''){
		 $.post("xascf/material/page.shtml",{fancingOrderNo:orderNo},function(data){
			var lostArry= personRootMaterialInfo;
