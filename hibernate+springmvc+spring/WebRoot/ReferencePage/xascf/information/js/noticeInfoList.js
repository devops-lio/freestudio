...
var _cols = [
    { title:'标题', name:'noticeTitle' ,width:180, align:'center', sortable: true, type: 'String'},
    ...
    { title:'类型', name:'noticeType' ,width:50, align:'center', sortable: true, type: 'String'},
...
var _mmg =  $("#mmg").mmGrid({
	...
	cols : _cols,
	url : 'xascf/information/noticeInfo/getNoticeInfoList.shtml',
	params : $("#noticeInfoListQueryForm").serialize(),
	method : 'post',
	checkCol : true,
	indexCol : true,
	indexColWidth : 15,
...
//				noticeInfoList.doubleClick(item);
//            	
//            });
 mmGridResizeListener(_mmg, $(".page-content"));           
var noticeInfoList = function() {
	return {
		/**查询*/
		query : function() {
            _mmg.load($("#noticeInfoListQueryForm").serialize());
		},

		/**双击*/
		doubleClick : function(item){
			var noticePid=item.noticePid;
			$.post("xascf/information/noticeInfo/toEditorPage.shtml",{noticePid:noticePid},function(data){
		...
		/**新增跳转至发布页面*/
		add : function() {
			$.post("xascf/information/noticeInfo/toEditorPage.shtml",{noticePid:""}, function(data) {
				$("#xascfMainPanel").empty();
				$("#xascfMainPanel").append(data);
			});
		},
		...
		edit : function(){
			...
			$.post("xascf/information/noticeInfo/toEditorPage.shtml",{noticePid:item.noticePid},function(data){
					$("#xascfMainPanel").empty();
					$("#xascfMainPanel").append(data);
			});
		},
		...
		*/
		publish : function(){
			...
			debugger
			if(item.status=="已发布"){
				PUI.MessageBox.alert("该条信息已发布，请重新选择！");
				return;
			}
			PUI.MessageBox.show({
				title: "发布公告信息",
		...
		*/
		...
		/**清空**/
		clear : function(){
			PUI.util.resetForm($("#noticeInfoListQueryForm"));
		}
	};
}();