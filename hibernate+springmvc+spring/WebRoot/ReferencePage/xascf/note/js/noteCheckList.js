 ...
 var ch_cols = [
       { title:'放款单号', name:'fancingOrderNo' ,width:120, align:'center', sortable: true, type: 'object',
	    ...
	    { title:'操作', name:'' ,width:40, align:'center', sortable: true, type: 'String',
		 	  renderer: function(val,item,rowIndex){
		 		 var orderNo=item.fancingOrderNo;
		 			 	return '<a class="btnPrice" href="javascript:void(0)" onclick="NoteChangeList.noteToChange(\''+orderNo+'\')">审核</a>';
	 	 }},
        ...
        { title:'有效票据折扣金额', name:'noteDisprice' ,width:120, align:'center', sortable: true, type: 'object',
	        	renderer: function(val){return formatMoney(val)+' 元'}},
	    { title:'最新票据状态', name:'noteStatus' ,width:70, align:'center', sortable: true, type: 'object',
		        	renderer: function(val){
		        		if(val=='0'){
     						val='失效';
     					}else
     						val='有效';
		        		return val}}, 
    	{ title:'最新票据到期日', name:'noteToDate' ,width:70, align:'center', sortable: true, type: 'object',
    		renderer: function(val){
    			var html='<span >'+val+'</span>';
    			if(val!='' && null!=val){
    				var  repayTime=getDate(val);
...
var	waitCheck_mmg =  $("#waitCheck_mmg").mmGrid({
		...
		cols : ch_cols,
		url : 'xascf/notePool/getFancingNoteChangeList.shtml?type=CHECK',
...
var NoteChangeList = function() {
	return {
		...
		noteToChange:function(orderNo){
			$.post("xascf/notePool/getAllNoteCheckDetailJsp.shtml",{fancingOrderNo:orderNo},function(data){ 
