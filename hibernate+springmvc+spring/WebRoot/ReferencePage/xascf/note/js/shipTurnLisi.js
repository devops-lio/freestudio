...
var shipTurnLisi_mmg=null;
var shipTurnLisi_cols=null;
var ShipTurnLisi = function(){	
	return {
		...
		shipTurnGrid:function(){
			shipTurnLisi_cols=[
			   ...
			   shipTurnLisi_mmg=$('#shipTurnLisi_mmg').mmGrid({
			    				width:'auto',
			    				height: 160,
			    				cols: shipTurnLisi_cols, 
			    				indexCol: false,
			    				indexColWidth: 30,
			    				fullWidthRows:true,
			    				multiSelect: true,
...
//			    	ShipTurnLisi.shipTurnCount();
		...
		shipTurnCount : function() {
			...
			var discountPrice=0;
			var termDate='';
			if (null != shipTurnLisi_mmg.row(0)) {
				for ( var i = 0; i < shipTurnLisi_mmg[0].rows.length; i++) {
					var price = parseFloat(shipTurnLisi_mmg.row(i).totalPrice);
					allnum += 1;
					allPrices += price;
					termDate=shipTurn_mmgLisi.row(i).toDate;
