...
public class SimpleHibernateDaoImpl<T, PK extends Serializable> {
	...
	public List<T> get(final Collection<PK> ids, final boolean isCache) {
		return find(isCache, Restrictions.in(getIdName(), ids));
	}
	...
	public List<T> getAll(String orderByProperty, boolean isAsc, boolean isCache) {
		Criteria c = createCriteria(isCache);
		if (isCache) {
			c.setCacheable(true);
		}
		if (isAsc) {
	...
	public T findUniqueBy(final String propertyName, final Object value,
		...
		Assert.hasText(propertyName, "propertyName不能为空");
		Criterion criterion = Restrictions.eq(propertyName, value);
		return (T) createCriteria(isCache, criterion).uniqueResult();
	}
	...
	public T findUnique(final boolean isCache, final Criterion... criterions) {
		return (T) createCriteria(isCache, criterions).uniqueResult();
	}
	...
	public boolean isPropertyUnique(final String propertyName,
		...
		Object object = findUniqueBy(propertyName, newValue, isCache);
		return (object == null);
	}
